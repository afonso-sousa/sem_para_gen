...
class BartLearnedPositionalEmbedding(nn.Embedding):
    ...
    def forward(
        self, input_ids: torch.Tensor, past_key_values_length: int = 0
    ):
...
class BartEncoder(BartPretrainedModel):
    ...
    def __init__(
        ...
        self.token_type_embeddings = nn.Embedding(
            config.type_vocab_size, embed_dim
        )
    ...
    def forward(
        ...
        attention_mask: Optional[torch.Tensor] = None,
        token_label_ids: Optional[torch.Tensor] = None,
        ...
        assert token_label_ids.max().item() < 23
        assert (
            token_label_ids.min().item() >= 0
        ), f"Min value: {token_label_ids.min().item()}"

        token_type_embeddings = self.token_type_embeddings(token_label_ids)
...
class TokenBartForConditionalGeneration(BartPretrainedModel):
